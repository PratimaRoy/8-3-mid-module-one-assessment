...
function filterByCountMinimum(animals, minimum) {
  let filterByCountMin =[];
  for(let animal of animals) {
    if(animal.count >= minimum) {
      filterByCountMin.push(animal);
    }
  }
  return filterByCountMin;
}
...
function getMostCommonAnimal(animals) {
  let mostCommAnimal = animals[0];
  for (let i = 0; i < animals.length; i++) {
    if (mostCommAnimal.count < animals[i].count) {
      mostCommAnimal = animals[i];
    }
  }
  return mostCommAnimal || null;
}
